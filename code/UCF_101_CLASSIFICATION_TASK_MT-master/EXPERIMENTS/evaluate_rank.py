...
def evaluate(file_patern_labels, file_patern_ids, path_to_output_representation, max_rank=1, debug=False):

	confusion = np.zeros((101,101))


	rank_k = range(1, max_rank+1)
	...
	for unique_id in unique_ids:
		...
		for rnk in rank_k:
			if(rnk < len(class_counts)):
				if( class_counts[rnk-1][1] == labels_true_curr_id[0]):
					acc_voting +=1
					break
			else:
				break
		...
		for rnk in rank_k:
			if( sum_curr_posteriors[-rnk] == labels_true_curr_id[0]):
				acc_sum +=1
				break


		confusion[labels_true_curr_id[0], sum_curr_posteriors[-1]] += 1
	...
	acc_voting = acc_voting/float(len(unique_ids))
	print '(ID-VOTING) Test acc : %.5f %%' % (acc_voting  * 100.0)

	acc_sum = acc_sum/float(len(unique_ids))
	print '(ID-SUM) Test acc : %.5f %%' % (acc_sum  * 100.0)

	print "Total # of features: " + str(labels_true.shape[0])
	print "Total # of videos: " + str(len(unique_ids))
	"""

	acc_sum = acc_sum/float(len(unique_ids))
	acc_voting = acc_voting/float(len(unique_ids))
	print '%d & - & %.5f & %.5f \\\\ ' % (max_rank,(acc_voting  * 100.0),(acc_sum  * 100.0))

	if(max_rank==1):
		np.save("./confusion_mat.npy", confusion)
...
if __name__ == "__main__":
	...
	if len(sys.argv) < 3:
		...
		for i in xrange(10):
			evaluate(file_pattern_labels, file_pattern_ids, path_to_output_representation, i+1, debug)
